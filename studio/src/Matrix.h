...
public:
	...
	Matrix(const Matrix& obj) {
		...
		col = obj.col;
		index = (double *)malloc( row*col*sizeof( double ));
		memcpy(index, obj.index, row*col* sizeof( double ));
	}
	// 二维数组构造
	Matrix(double **array, int X, int Y) {
		...
		index = (double *)malloc( row*col*sizeof( double ));
		for (int i = 0; i < row; ++i){
			memcpy(index + i*col, array[i], col*sizeof( double ));
		}
	}
	// 连续数组构造
	Matrix(double *array, int X, int Y) {
		...
		col = Y;
		index = (double *)malloc( row*col*sizeof( double ));
		memcpy(index, array, row*col* sizeof( double ));
	}
	// 复制构造
	...
	// 单位阵初始化
	void E() {
		memset(index, 0, row*col*sizeof(double));
		for (int i = 0; i < row; ++i) {
			index[i + i*col] = 1;
		}
	}
	...
	Matrix block(int X, int Y, int new_row, int new_col) {
		...
		for (int i = 0; i < new_row; ++i) {
			memcpy(tmp.index + i*new_col, index + (i + X)*col + Y, new_col* sizeof( double ));
		}
		return tmp;
	}
	// 矩阵赋值
	void cpy(Matrix obj, int X, int Y) {
		for (int i = 0; i < obj.row; ++i) {
			memcpy(index + (i + X)*col + Y, obj.index + i*obj.col, obj.col* sizeof( double ));
		}
	}
	//行交换
	void swaprow(const int X, const int Y) {
		double* tmp = (double *)malloc( col*sizeof( double ));
		memcpy(tmp, index + X*col, col* sizeof( double ));
		memcpy(index + X*col, index + Y*col, col* sizeof( double ));
		memcpy(index + Y*col, tmp, col* sizeof( double ));
	}
	//行变换
	void rowmultik(int X, double Y) {
		for (int i = 0; i < col; ++i) {
			index[X*col + i] *= Y;
		}
	}
	...
	Matrix solve(const Matrix B) {
		...
		for (i = 0; i < A.row; ++i) {
			...
			//将对角位置转换为1
			double tmp = 1.0 / A.index[i*col + i];
			X.rowmultik(i, tmp);
			A.rowmultik(i, tmp);
			//将该列非对角位置转换为0
