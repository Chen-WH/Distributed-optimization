        ...
        for (int j = 0; j < 4; ++j){
            if (i == j){
                ...
                if (j < i){
                    ...
                    lap(i, j) = -c1*exp(-(psi(i, j) - Phi(i))*(psi(i, j) - Phi(i))*4/pi/pi) - c2*exp(-(Phi(i))*(Phi(i))*4/pi/pi)\
                     - c3*exp(-(fabs(Theta(j) - Theta(i)) - pi/2)*(fabs(Theta(j) - Theta(i)) - pi/2)*36/pi/pi)\
                     -c1*exp(-(psi(j, i) - Phi(j))*(psi(j, i) - Phi(j))*4/pi/pi) - c2*exp(-(Phi(j))*(Phi(j))*4/pi/pi)\
                     - c3*exp(-(fabs(Theta(j) - Theta(i)) - pi/2)*(fabs(Theta(j) - Theta(i)) - pi/2)*36/pi/pi);
                }
            }
        }
